...
const our_prices = {
    "彰化足印": {"早餐": 300, "午餐": 600, "晚餐": 600},
    "寵愛之名": {"早餐": 300, "午餐": 600, "晚餐": 600},
    "遇見好心": {"早餐": 300, "午餐": 600, "晚餐": 600},
    "龍盈新春": {"早餐": 300, "午餐": 650, "晚餐": 650},
...
function calculateTotalCost(option, hospital, activity, hospitalMeals, ourMeals, refund, discount) {
    let total = 0;
    if (option === "1" || option === "2") {
        total += calculateCost(hospital_prices[hospital], hospitalMeals);
    }
    if (option === "2" || option === "3") {
        total += calculateCost(our_prices[activity], ourMeals);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    calculateButton.addEventListener('click', function() {
        ...
        const totalCost = calculateTotalCost(option, hospital, activity, hospitalMeals, ourMeals, refund, discount);
        
        let resultHTML = `<h3>計算結果</h3>`;
        resultHTML += `原始費用：${totalCost + refund + discount} 元<br>`;
        if (refund > 0) {
